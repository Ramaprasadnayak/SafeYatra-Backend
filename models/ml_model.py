...
class SafeYatraRiskModel:
    def __init__(self, model_path="safeyatra_risk_model.joblib"):
        ...
        self.district_lookup["_key"] = (
            self.district_lookup["district_name"].str.strip().str.lower()
        )
    def _get_row(self, district_name, state_name=None):
        key = district_name.strip().lower()
        matches = self.district_lookup[self.district_lookup["_key"] == key]
        if state_name is not None:
            matches = matches[
                matches["state_name"].str.lower() == state_name.strip().lower()
            ]
        if matches.empty:
            raise ValueError(
                f"No district found matching '{district_name}'"
                + (f" in state '{state_name}'" if state_name else "")
            )
        if len(matches) > 1:
            options = ", ".join(
                f"{r.district_name} ({r.state_name})" for r in matches.itertuples()
            )
            raise ValueError(
                f"Multiple districts named '{district_name}' found: {options}. "
                "Pass state_name= to disambiguate."
            )
    ...
    def _risk_score(self, row):
        ...
        for rate_col, weight in self.weights.items():
            ...
            norm_val = (row[rate_col] - cmin) / (cmax - cmin + 1e-9)
    ...
    def predict(self, district_name, state_name=None):
        ...
        feature_vec = pd.DataFrame([row[self.features].astype(float)])
        pred_idx = int(self.rf_model.predict(feature_vec)[0])
...
if __name__ == "__main__":
    ...
    result = model.predict("Rajsamand")
    print(result)
